...
public class CuotaD extends JInternalFrame implements ActionListener
{
    //variables globales
    ...
    public void crearInterfaz()
    {
        ...
        Txmx1.addKeyListener(new KeyListener()
                                 {
                                     public void keyTyped(KeyEvent e) {
                                         if (Txmx1.getText().length()== 5) e.consume();
                                         char c= e.getKeyChar();
        ...
        Txmx2.addKeyListener(new KeyListener()
                                 {
                                     public void keyTyped(KeyEvent e) {if (Txmx2.getText().length()== 5) e.consume();
                                     char c= e.getKeyChar();
                                         if (((c < '0') || (c > '9')) && (c != KeyEvent.VK_BACK_SPACE)) {
                                            e.consume();}}
                                     public void keyPressed(KeyEvent arg0) {} 
    ...
    private void cargar_data() 
    {
        try
        {
            ...
            ResultSet rs=sentencia.executeQuery("select * from cuotas where cvecuota='1'");
                ...
                if(clave.equals(""))
                    JOptionPane.showMessageDialog(null, "No se ha podido cargar los datos","Cuota Doméstica",JOptionPane.ERROR_MESSAGE);
                else
                    mostrar();
            con.close();
            sentencia.close();
    ...
    public void guardar()
    {
        ...
        if(!Txmx1.equals("") && !Txmx2.equals("") && !Txmx3.equals("") && !Txmx4.equals("") && !Txt1.equals("") && !Txt2.equals("") && !Txt3.equals("") && !Txt4.equals(""))
            ...
            JOptionPane.showMessageDialog(null, "Asegúrese de llenar todos los campos","Cuota Doméstica",JOptionPane.ERROR_MESSAGE);
        if(bandera)
        {
            int filas=0;
            try
            {
                String actualiza="update cuotas set max1="+Txmx1.getText().trim()+",max2="+Txmx2.getText().trim()+",max3="+Txmx3.getText().trim()+",max4="+Txmx4.getText().trim()+",tari1="+Txt1.getText().trim()+",tari2="+Txt2.getText().trim()+",tari3="+Txt3.getText().trim()+",tari4="+Txt4.getText().trim()+" where cvecuota='1'";
                Connection con=DriverManager.getConnection("jdbc:sqlserver://localhost:1433;databaseName=SMAPASab","sa","sa");
                Statement sentencia=con.createStatement();
                System.out.print(filas);
                filas=sentencia.executeUpdate(actualiza);
                if(filas>0){
                    JOptionPane.showMessageDialog(null, "Guardado completo","Cuota Doméstica",JOptionPane.INFORMATION_MESSAGE);
                    ...
                    JOptionPane.showMessageDialog(null, "No se pudo guardar","Cuota Doméstica",JOptionPane.ERROR_MESSAGE);
            }
