...
public class anticipo extends JInternalFrame implements ActionListener
{
    ...
    JTextField Txtcuenta,Txtname,txttari,txtcosto,txtAnti;
...
////////////    boolean duda_exitia=false; //saber si ya existia registro de algun pago del usuario
    ...
    private void crearInterfaz() 
    {
        ...
        txtAnti=new JTextField();
        txtAnti.setSize(150,20);
        txtAnti.setLocation(90, 130);
        txtAnti.setEnabled(true);
        txtAnti.setBorder(border);
        ...
        txtcosto.setSize(150,20);
        txtcosto.setLocation(90, 190);
    ...
    public void cargarNombres()
    {
      ...
      if(numCuentas!=0)
      {
        try
        {
            ...
            Connection con=DriverManager.getConnection("jdbc:sqlserver://localhost:1433;databaseName=SMAPASab","sa","sa");
            Statement sentencia=con.createStatement();
            ResultSet rs=sentencia.executeQuery("select u.nombre name,u.apellidos apell from usuario u");
                while(rs.next())
                {   
...
//    
    public void buscar()
    {
      ...
      //int anti=getAnti();
        ...
        {
            if(opcion==1)
                query="select usuario.nombre+'-'+usuario.apellidos name,usuario.cuenta cue,cuotas.cvecuota tar, usuario.domicilio+' '+usuario.numero doc from usuario inner join cuotas on cuotas.cvecuota=usuario.cveTar where usuario.nombre="+clave;
            if (opcion==2)
                query="select usuario.nombre+'-'+usuario.apellidos name,usuario.cuenta cue,cuotas.cvecuota tar, usuario.domicilio+' '+usuario.numero doc from usuario inner join cuotas on cuotas.cvecuota=usuario.cveTar where usuario.cuenta='"+clave+"'";
            Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver"); //Cargar driver
            Connection con=DriverManager.getConnection("jdbc:sqlserver://localhost:1433;databaseName=SMAPASab","sa","sa");
            Statement sentencia=con.createStatement();
            ResultSet rs=sentencia.executeQuery(query);
                ...
                {   
                    ...
                    txtcosto.setText(rs.getString("doc"));
...
//  
      private int existe()
      {
        ...
        try
        {
            ...
            Connection con=DriverManager.getConnection("jdbc:sqlserver://localhost:1433;databaseName=SMAPASab","sa","sa");
            Statement sentencia=con.createStatement();
            ResultSet rs=sentencia.executeQuery("select cantidad from anticipo where cuenta='"+Txtcuenta.getText()+"'");
                while(rs.next())
                {   
      ...
      {
          if(!txtAnti.getText().equals("") && !Txtcuenta.getText().equals(""))
          {
            int filas=0;
            int duda_exitia=existe();
            String actualizar="";
            try
            {
                if(duda_exitia==1)
                    actualizar="update anticipo set cantidad =(select cantidad from anticipo where cuenta='"+Txtcuenta.getText()+"')+"+txtAnti.getText().trim()+";";
                else
                    actualizar="insert into anticipo values('"+Txtcuenta.getText().trim()+"',"+txtAnti.getText().trim()+");";
                Connection con=DriverManager.getConnection("jdbc:sqlserver://localhost:1433;databaseName=SMAPASab","sa","sa");
                Statement sentencia=con.createStatement();
          ...
          {
             JOptionPane.showMessageDialog(null, "Complete todos los campos","Anticipo",JOptionPane.ERROR_MESSAGE);
          }
